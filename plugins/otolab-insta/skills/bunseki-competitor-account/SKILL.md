...
分析が終わったら、台本生成用の **`style_profile.md`** を出力フォルダ直下に保存する（**daihon-copy-makeスキルがこれを読んでユーザーの台本を作る**ための引き継ぎファイル）。含める内容：一言コンセプト／フックの型（実例つき・変形パターンも）＋⭐**冒頭1文の同梱情報の分解**（原文の1文目をそのまま引用し、属性／年齢／関係／経過年数／事件のどれが1文に入っているかを書く。要素名だけだと台本側で情報が落ちて「〇〇に言われた。」級の情報量ゼロの冒頭になる）／展開のブロック構造＋⭐**「これは〜の話」系の宣言・決意が何秒地点か**（前倒し防止）／⭐**生の台詞と反復回数**（鉤括弧の中身そのまま・同じ台詞を何回繰り返すか。回数まで書く）／オチの型／**視聴者心理の設計（心理レバー・効果ベース＝どこで何を感じさせ、伏線をどこで回収し、どんな感情の弧か。推定と明示）**／文体ルール（人称・語尾・頻出フレーズ・迂回表現）＋**1本の中で統一されているか**（方言か標準語か／話し言葉か書き言葉か）／尺と文字数（**空白を除いた実測文字数を数字で**。台本側が±20%で合わせるため）／参考にすべき文字起こしファイル名2〜3本／移植時の注意（固有ギャグの丸写し禁止）／末尾に1行「台本を書くときはこのファイルと**原文の文字起こしの両方**を読むこと（要約だけからは言い回しとリズムが再現できない）」。報告書セクション5と同じ内容だが、機械可読なプレーンMarkdownで書く。書き方の見本は `<skill>/references/style_profile_example.md`（架空アカウントの記入例）
