...
分析結果を台本づくりに使う場合は、報告書完成後に 〔発信者専用の台本スキル〕 / 〔発信者専用の台本スキル〕 等の該当スキルへ引き継ぐ。
...
### 文字起こしが使えるか自動判定（ナレーションなし・テロップ主体アカウント対策）

料理・実演系には**ナレーションゼロでBGM＋画面テロップだけ**のアカウントが多い。その場合、音声文字起こしは「ご視聴ありがとうございました」等のBGM幻覚になり内容分析に使えない。パイプライン完了後、必ず判定する：

```bash
python3 <skill>/scripts/detect_narration.py reels/
```
...
**C. 台本の共通構成** — transcripts/ の全文（テロップ主体アカウントなら frames/ から実読したテロップ列）から：冒頭フックの型（0〜3秒）／展開のブロック構造／オチ・締めの型／口調・語尾・頻出フレーズ／尺と文字数の目安。**テロップ主体の場合は「1ショット1テロップ」の並びが台本**なので、その順序と、途中に差す根拠・材料公開・回遊締めまで型として書く。〔発信者専用の台本スキル〕を作ったときのように「この人の型」をテンプレート化する。**あわせて「視聴者心理の設計」**（[[reference-viewer-psychology-blueprint]]）＝各パートが視聴者に起こす効果（どこで疑問/伏線を植え、どこで回収するか、感情の弧）を**効果ベースで**書く。**制作者の内心は断定せず、視聴者側の反応の推定として明示**する（「良いフック」で止めず、なぜ効くか＝移植可能なメカニズムまで）
