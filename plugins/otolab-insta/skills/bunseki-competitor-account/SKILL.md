...
2. `get_page_text`で収集し `data/profile.json` に保存：表示名・@ユーザー名・フォロワー数・投稿数・bio全文・ハイライト名・外部リンク。**外部リンクはリンク先の中身まで見る**（リットリンク等のまとめリンクは開いて、何に着地するか＝楽天ROOM/Amazon/講座LP/LINE登録などを控える。STEP 5-Eの収益化導線分析の材料）
...
**最初にバズ判定をする（このアカウントを教科書にしてよいかの前提チェック）。** 基準は buzz-kikaku-research と同一（どのスキルでも受講生が同じ物差しを学べるように、変えない）。STEP 1のフォロワー数と stats.json（中央値・最高値）から判定する：

- **バズ認定ライン** = 再生数がフォロワー数の10倍以上、または30万回再生以上。各リールが「フォロワー×何倍か」も計算する（平常値=中央値比と併記する）
- **影響力型アカウントの罠** = フォロワー10万人以上 かつ 中央値がフォロワー数×2以上 かつ 最高再生数が中央値の3倍以下（どの投稿も満遍なく高く、突出した1本がない）→ 伸びの主因は「本人の人気」の可能性が高い。**該当したら報告書の①数字セクションで数値根拠つきで警告し、F（活かし方）は企画・フック・台本の丸ごと移植ではなく、世界観・編集・サムネ設計など再現可能な要素に限定する**（分析自体は続行してよい）

実データだけを根拠にする。見ていないものを「〜のはず」と書かない。以下の6つの柱で抽出する：
...
**C. 台本の共通構成** — transcripts/ の全文（テロップ主体アカウントなら frames/ から実読したテロップ列）から：冒頭フックの型（最初の1〜2秒で手を止める仕掛け＋〜7秒の見続ける理由。**ビジュアル／サウンド／バーバルの3フック分類**で書き、バーバルは7トリガー＝検証企画/常識破壊/ビフォーアフター/権威×論争/プライベート公開/結末予想不能/ランキング のどれかも添える）／展開のブロック構造／オチ・締めの型／口調・語尾・頻出フレーズ／尺と文字数の目安。**テロップ主体の場合は「1ショット1テロップ」の並びが台本**なので、その順序と、途中に差す根拠・材料公開・回遊締めまで型として書く。〔発信者専用の台本スキル〕を作ったときのように「この人の型」をテンプレート化する。**あわせて「視聴者心理の設計」**（[[reference-viewer-psychology-blueprint]]）＝各パートが視聴者に起こす効果（どこで疑問/伏線を植え、どこで回収するか、感情の弧）を**効果ベースで**書く。**制作者の内心は断定せず、視聴者側の反応の推定として明示**する（「良いフック」で止めず、なぜ効くか＝移植可能なメカニズムまで）

**D. 伸びの構造** — stats.jsonの平常値（中央値）を基準に、各リールが何倍かを計算（フォロワー×何倍かも併記）。バズ回のテーマ・サムネ・フックの共通点。最新5本とバズ回の違い（運用の変化）。**投稿日も見る**：選定リールの投稿日は `reels/<ID>.info.json` の `upload_date` にある。**1年以上前に伸びたリールは今のアルゴリズムの根拠として弱い**ので、バズ回が古い場合はその旨を明記し、直近の伸びと分けて扱う。**あわせて「なぜ伸びているか」を4要素で診断する**：世界観🎨／企画力💡／編集力✂️／独自性✨ を◎○△で採点し、**どの1〜2要素で100点を取っているか**を明記する（伸びるアカウントは1〜2要素が突出していて他は普通でも伸びる。全部60点は伸びない）

**E. 収益化導線** — この人が何で稼いでいるか、リールをどこに着地させているかを「形跡」から読む：楽天ROOM・Amazonリンク→購入型の紹介収入／「PR」「タイアップ」表記→企業案件／ハイライトの「講座」「LINE登録」「質問箱」→自社商品・講座／まとめリンクはリンク先まで見る。リール→プロフィール→着地先の流れを1本の線で書く。**フォロワーが多いのに収益化の形跡がない場合は「収益化はまだの可能性」と添える**（断定はしない）。形跡が読めない場合は「確認できなかった」と正直に書く

**F. 活かし方** — 明日から真似できる具体アクション4〜6個。**サムネ／フック／台本構成（展開ブロックの組み方・オチの型）／企画選定／数値の見方の5観点をひと通りカバーする**こと。特に台本構成はCで抽出した「この人の型」をユーザーの台本づくりにどう移植するかまで具体化する（ここが抜けやすい）。**丸パクリではなく構造の移植**を勧めること。
...
分析が終わったら、台本生成用の **`style_profile.md`** を出力フォルダ直下に保存する（**daihon-copy-makeスキルがこれを読んでユーザーの台本を作る**ための引き継ぎファイル）。含める内容：一言コンセプト／フックの型（実例つき・変形パターンも）＋⭐**冒頭1文の同梱情報の分解**（原文の1文目をそのまま引用し、属性／年齢／関係／経過年数／事件のどれが1文に入っているかを書く。要素名だけだと台本側で情報が落ちて「〇〇に言われた。」級の情報量ゼロの冒頭になる）／展開のブロック構造＋⭐**「これは〜の話」系の宣言・決意が何秒地点か**（前倒し防止）／⭐**生の台詞と反復回数**（鉤括弧の中身そのまま・同じ台詞を何回繰り返すか。回数まで書く）／オチの型／**視聴者心理の設計（心理レバー・効果ベース＝どこで何を感じさせ、伏線をどこで回収し、どんな感情の弧か。推定と明示）**／文体ルール（人称・語尾・頻出フレーズ・迂回表現）＋**1本の中で揃っているか、崩れていくか**（方言か標準語か／話し言葉か書き言葉か。⚠️ **揃っている＝良い、混在＝悪ではない。** 丁寧語から崩れていく（「〜です」→「戻らん」→「うっざ」）のは**感情で仮面が剥がれる設計**で、共感・独白系では主武器。揃っているのか、どこで何に崩れるのかを**そのまま記録する**——崩れ方も型）／尺と文字数（**空白を除いた実測文字数を数字で**。台本側が±20%で合わせるため）／参考にすべき文字起こしファイル名2〜3本／移植時の注意（固有ギャグの丸写し禁止）／末尾に1行「台本を書くときはこのファイルと**原文の文字起こしの両方**を読むこと（要約だけからは言い回しとリズムが再現できない）」。報告書セクション5と同じ内容だが、機械可読なプレーンMarkdownで書く。書き方の見本は `<skill>/references/style_profile_example.md`（架空アカウントの記入例）
